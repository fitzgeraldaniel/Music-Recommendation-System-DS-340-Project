...
class MUSE_Trainer(Embedder):
    ...
    def calculate_loss(self, predictions, batch):
        ...
        labels = batch['labels'].view(-1)  # Flatten in case it's not 1D
        print(f"logits shape: {logits.shape}")  # For debugging
        print(f"batch labels shape: {labels.shape}")  # For debugging

        loss = self.loss_func(logits, labels)  # Ensure loss_func is compatible
        return loss
